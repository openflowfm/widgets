...
`bench/`, served by `vite.config.ts` in the standalone Widgets repository. A page
of every control in every state, with no app around it.

```sh
npm ci
npm run dev              # the bench alone, http://localhost:5273
```

The application repository runs its own device bench with `npm run dev:devices`.
The widget bench has its own installation and server lifecycle.
...
The subtler one: **each Vite server names its own `cacheDir`.** When multiple configs share an installation, the default can resolve to the same `node_modules/.vite`, and Vite hashes the config into that
...
to happen. `set/` uses `node_modules/.vite/set`, the widget bench uses its own repository’s `node_modules/.vite/bench`, and
...
It also proves the boundary. The bench imports `src` and nothing else — no bridge,
...
reasoned about in
[set/docs/device-faces.md](https://github.com/ryangavin/better-session-view/blob/main/set/docs/device-faces.md). What the bench owes it is the
...
The bench is not shipped in the package. CI bundles it as a smoke check, and
`npm run typecheck` covers it. `npm run build` emits library declarations only.
