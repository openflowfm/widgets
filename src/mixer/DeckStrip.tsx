import { identityLabel } from '../theme/resolve.ts';
import { Separator, MixerSection } from './Separator.tsx';
import { ButtonFace } from '../controls/ButtonFace.tsx';
import { ContextControls, PhonesIcon } from './ContextControls.tsx';
import { Momentary } from './Momentary.tsx';
import { type CSSProperties } from 'react';
import { Button } from '../controls/Button.tsx';
import { Toggle } from '../controls/Toggle.tsx';
import { Knob } from '../controls/Knob.tsx';
import { Slider } from '../controls/Slider.tsx';
import { Segmented } from '../controls/Segmented.tsx';
import type { MixerDeck, MixerViewProps } from './model.ts';
import { KnobStack } from './KnobStack.tsx';
import { FrameMeter } from './frames.tsx';

/** Beats read as bars where they divide into them: what a DJ counts. */
export const beatsText = (beats: number) => beats % 4 === 0 ? `${beats / 4} ${beats === 4 ? 'bar' : 'bars'} · ${beats} beats` : `${beats} ${beats === 1 ? 'beat' : 'beats'}`;

export function DeckStrip({ deck: d, index, commands, readFrame, theme, params, deckProps, loopBeats }: Pick<MixerViewProps, 'commands' | 'readFrame' | 'theme' | 'params' | 'deckProps'> & { deck: MixerDeck; index: number; loopBeats: number }) {
  const { level: LEVEL, trim: TRIM, send: SEND, eq: EQ, filter: FILTER } = params;
  const ROUTE = ['A', 'Thru', 'B'];
  const hostProps = deckProps?.(d.id);
  return <div {...hostProps} className={`play-deck ${hostProps?.className ?? ''}`} style={{ ...hostProps?.style, '--play-stems': d.stems.length, '--deck-ink': theme.decks[d.id]?.ink ?? theme.primary } as CSSProperties}>
        <div className="play-track"><b className="play-letter">{d.letter}</b><div><h3>{d.track?.title ?? 'Empty deck'}</h3><p>{d.track?.artist ?? d.message ?? d.status}</p></div><span>{d.track?.bpm != null && Number.isFinite(d.track.bpm) && d.track.bpm > 0 ? `${Math.round(d.track.bpm)} BPM` : null}<br />{d.syncLeader?'LEADER':d.track?.key ?? ''}</span></div>

        <MixerSection className="play-performance">
          <fieldset className="play-grid" data-full={d.full} disabled={d.status !== 'ready'}>
          <Separator orientation="vertical" className="play-hot-cue-divider"/><span className="play-axis">Hot cue</span><span aria-hidden="true" />{d.stems.map((stem, i) => <span className="play-stem-name" style={{ '--stem-ink': theme.stems[stem.id] ?? theme.primary, '--stem-label': identityLabel(theme.stems[stem.id] ?? theme.primary) } as CSSProperties} key={stem.id}>{stem.name}</span>)}
          {d.sections.map(section => <div className="play-launch-row" key={section.id}>
            {d.full ? <Toggle title="Hot cue: jump to this section and continue playing" on={d.fullSection === section.id} label={`Deck ${index + 1}: launch ${section.name} full mix${d.fullQueued === section.id ? ', queued' : ''}`} onChange={() => commands.launch(d.id, section.id)} hint="Jumps this deck to that part of the song and keeps playing. It waits for the next bar unless launch timing is set to Now.">{section.name}</Toggle> : <Button label={`Deck ${index + 1}: launch ${section.name} all stems`} title="Hot cue: jump to this section and continue playing" onPress={() => commands.launch(d.id, section.id)} hint="Jumps every stem on this deck to that part of the song and keeps playing. It waits for the next bar unless launch timing is set to Now.">{section.name}</Button>}<span aria-hidden="true" />
            {d.stems.map((stem, s) => {
              const active = stem.selected === section.id, queued = stem.queued === section.id;
              return <div className="play-cell" key={stem.id} data-active={active} data-queued={queued} style={{ '--stem-ink': theme.stems[stem.id] ?? theme.primary, '--stem-label': identityLabel(theme.stems[stem.id] ?? theme.primary) } as CSSProperties}>
                <Toggle disabled={d.status !== 'ready' || !stem.available} on={active} ink={theme.stems[stem.id] ?? theme.primary} title="Loop this section on this stem" width={34} label={`Deck ${index + 1}: ${section.name} ${stem.name}${queued ? ', queued' : active ? ', selected' : ''}`} onChange={() => commands.launch(d.id, section.id, stem.id)} hint={`Loops that part of the song on ${stem.name.toLowerCase()} alone. The deck's other stems stay wherever they are, which is how a part is built up piece by piece.`}>{queued ? '◷' : active ? '▶' : '▷'}</Toggle>
              </div>;
            })}
          </div>)}
          <Button label={`Deck ${index + 1}: stop all stems`} onPress={() => commands.launch(d.id, null)} hint="Stops every stem on this deck at the next launch point. The other decks carry on.">{(d.full ? d.fullQueued === null : d.stems.every(stem => stem.queued === null)) ? '◷ Stop' : 'Stop'}</Button><span aria-hidden="true" />{d.stems.map((stem, i) => <Button disabled={d.status !== 'ready' || !stem.available} key={stem.id} label={`Deck ${index + 1}: stop ${stem.name}`} width={34} onPress={() => commands.launch(d.id, null, stem.id)} hint={`Stops ${stem.name.toLowerCase()} on this deck and leaves its other stems playing.`}>■</Button>)}
        </fieldset>
          <div className="play-deck-status">{d.status!=='empty' && d.status!=='loading' && d.message && <span role="status">{d.message}</span>}</div>
          <div className="play-deck-loops" role="group" aria-label={`Deck ${index + 1} loop`}>
            <Button disabled={d.status !== 'ready' || !commands.deckLoopIn} label={`Deck ${index + 1} loop in`} title="Mark loop start and Cue" onPress={() => commands.deckLoopIn?.(d.id)} hint="Marks the start of a loop where this deck is playing now. Press Out to close it and the deck begins repeating. Pressed again inside a running loop, it brings the loop\u2019s front up to here.">In</Button>
            <Button disabled={!d.canLoopOut || !commands.deckLoopOut} label={`Deck ${index + 1} loop out`} title="Mark loop end and repeat" onPress={() => commands.deckLoopOut?.(d.id)} hint="Closes the loop here and starts it repeating. Pressed again inside a running loop, it pulls the loop\u2019s tail back to here.">Out</Button>
            <Toggle disabled={d.status !== 'ready' || d.loop?.end == null || !commands.setDeckLoopEnabled} on={d.loop?.enabled ?? false} label={`Deck ${index + 1} loop enabled`} title={d.loop?.enabled?'Exit loop, preserving its region':'Reloop saved region'} onChange={on => commands.setDeckLoopEnabled?.(d.id,on)} hint="Leaves the loop and plays on, or drops back into the loop this deck has saved. The loop is kept either way.">{d.loop?.enabled ? 'Exit' : d.loop?.start != null && d.loop.end == null ? 'Out…' : '↻'}</Toggle>
            {commands.resizeLoop && <div className="wdg wdg-control-group play-loop-move" role="group" aria-label={`Deck ${index+1} loop length`}>
              <ButtonFace size="medium" disabled={!d.loop?.enabled || !d.gridAvailable} aria-label={`Deck ${index+1} halve loop`} title="Halve the loop, keeping In" onClick={()=>commands.resizeLoop?.(d.id,.5)}>½</ButtonFace>
              <ButtonFace size="medium" disabled={!d.loop?.enabled || !d.gridAvailable} aria-label={`Deck ${index+1} double loop`} title="Double the loop, keeping In" onClick={()=>commands.resizeLoop?.(d.id,2)}>2×</ButtonFace>
            </div>}
            {commands.moveLoop && <div className="wdg wdg-control-group play-loop-move" role="group" aria-label={`Deck ${index+1} move loop`}>
              <ButtonFace size="medium" disabled={!d.loop?.enabled || !d.gridAvailable} aria-label={`Deck ${index+1} move loop back`} title="Slide the loop back one beat, leaving the playhead" onClick={()=>commands.moveLoop?.(d.id,-1)}>←</ButtonFace>
              <ButtonFace size="medium" disabled={!d.loop?.enabled || !d.gridAvailable} aria-label={`Deck ${index+1} move loop ahead`} title="Slide the loop ahead one beat, leaving the playhead" onClick={()=>commands.moveLoop?.(d.id,1)}>→</ButtonFace>
            </div>}
            <div className="play-quick-loop-group">{commands.quickLoop && <ButtonFace size="medium" className="play-quick-loop" disabled={!d.gridAvailable} aria-label={`Deck ${index+1} quick loop`} title={`${d.loop?.enabled?'Exit loop':'Quick loop'} · ${beatsText(loopBeats)}`} aria-pressed={d.loop?.enabled ?? false} onClick={()=>commands.quickLoop?.(d.id)}>↻ {loopBeats}</ButtonFace>}
            {commands.setSlip && <ContextControls label={`Deck ${index+1} loop settings`} face="···" active={!!d.slip} title={`Loop settings${d.slip?' · Slip on':''}`}>
              <label>Loop target<select aria-label={`Deck ${index+1} loop target`} disabled={d.loop?.start!=null && d.loop.end===null} value={d.loopFocus?'focus':'deck'} onChange={e=>commands.setLoopFocus?.(d.id,e.target.value==='focus')}><option value="deck">Active stems</option><option value="focus">Focus only</option></select></label>
              <label className="play-context-check"><input type="checkbox" aria-label={`Deck ${index+1} Slip loops`} checked={d.slip ?? false} onChange={()=>commands.setSlip?.(d.id,!d.slip)}/>Slip loops</label>
            </ContextControls>}</div>
          </div>


        </MixerSection>
        <MixerSection className="play-effects">
          <Knob ink="var(--amber)" name="FX A" label={`Deck ${index + 1} effects send A`} param={SEND} value={d.sendA} onChange={value => commands.setDeck(d.id, 'sendA', value)} hint="How much of this deck reaches the shared FX A effect. Taken after the fader, so it falls away as you fade the deck out." />
          <Separator orientation="vertical" className="play-column-rule"/><div className="play-filter-pair"><Knob ink="var(--amber)" name="Filter" label={`Deck ${index + 1} filter`} param={FILTER} value={d.filter} onChange={value => commands.setDeck(d.id, 'filter', value)} hint="Sweeps a filter across this deck. Left takes the top off, right takes the bottom out, and the middle is untouched." />
            {params.filterResonance && <Knob ink="var(--amber)" name="Resonance" label={`Deck ${index + 1} filter resonance`} param={params.filterResonance} value={d.filterResonance ?? 0} onChange={value => commands.setDeck(d.id, 'filterResonance', value)} hint="Emphasizes the filter cutoff on this deck. At center, Filter stays neutral. Resonance can raise peaks; lower Trim if needed." />}
          </div>
          <Separator orientation="vertical" className="play-column-rule"/><Knob ink="var(--amber)" name="FX B" label={`Deck ${index + 1} effects send B`} param={SEND} value={d.sendB} onChange={value => commands.setDeck(d.id, 'sendB', value)} hint="How much of this deck reaches the shared FX B effect. Taken after the fader, so it falls away as you fade the deck out." />
        </MixerSection>
        <MixerSection className="play-channel">
          <KnobStack stems full={d.full}>{d.stems.map(stem=><Knob key={stem.id} disabled={d.full || d.status !== 'ready' || !stem.available} name={stem.name} label={`Deck ${index + 1} ${stem.name} level`} param={params.stemLevel ?? LEVEL} value={stem.level} display={params.stemLevelDisplay?.(stem.level)} onChange={value=>commands.setStemLevel(d.id,stem.id,value)} ink={theme.stems[stem.id] ?? theme.primary} hint={`How loud ${stem.name.toLowerCase()} is inside this deck, before the deck fader. Turn it down to play the song without it.`}/>)}</KnobStack>
          <Separator orientation="vertical" className="play-column-rule"/>
          <div className="play-level-stack">
          <div className="play-channel-fader"><Slider name="" showValue={false} label={`Deck ${index + 1} level`} param={LEVEL} value={d.gain} onChange={value => commands.setDeck(d.id, 'gain', value)} length="auto" hint="This deck's level in the mix. The top of its travel is unity, so it only ever takes away — Trim is where a quiet track gets louder." />
          {((['left','right'] as const)).map((side,channel) => <div className="play-meter-column" key={side}><FrameMeter label={`Deck ${index + 1} ${side} output`} sample={() => readFrame().decks[d.id]?.stereo?.[channel] ?? 0} /><span>{side === 'left' ? 'L' : 'R'}</span></div>)}</div></div>
          <Separator orientation="vertical" className="play-column-rule"/>
          <KnobStack>
            <Knob className="play-trim" ink="var(--amber)" name="Trim" label={`Deck ${index + 1} trim`} param={TRIM} origin="center" value={d.trim} onChange={value => commands.setDeck(d.id, 'trim', value)} hint="Lifts or cuts the deck before everything else, from −24 to +12 dB. This is what matches a quiet record to a loud one, not the fader." />
            {['High', 'Mid', 'Low'].map((name, e) => <Knob key={name} name={name} label={`Deck ${index + 1} ${name}`} param={EQ} origin="center" value={d.eq[e]} onChange={value => commands.setDeckEq(d.id, e, value)} hint={`Lifts or cuts this deck's ${name.toLowerCase()} band. It rests at 0 dB, which is the record as it was, and cuts much further than it lifts.`} />)}
          </KnobStack>
        </MixerSection>
        <div className="play-deck-footer"><div className="play-routing-section"><Separator/><div className="play-route"><Toggle disabled={d.status !== 'ready' || !d.stems.some(stem => stem.available)} on={d.full} label={`Deck ${index + 1} original full mix`} title="Use the original unseparated track instead of stems" onChange={value => commands.setDeck(d.id, 'full', value)} hint="Plays the original recording instead of its separated stems. The stem controls go quiet, since there are no stems to move.">Full</Toggle><Separator orientation="vertical" className="play-column-rule"/><Segmented name="" label={`Deck ${index + 1} crossfade assignment`} items={ROUTE} index={d.route} onChange={value => commands.setDeck(d.id, 'route', value)} hint="Which side of the crossfader this deck answers to. Thru takes it off the crossfader altogether, so it plays wherever the crossfader sits." /><Separator orientation="vertical" className="play-column-rule"/><Toggle on={d.cue} onChange={value => commands.setDeck(d.id, 'cue', value)} label={`Deck ${index + 1} headphone cue`} title="Headphone monitoring" hint="Sends this deck to the headphones only. It is taken before the fader and the crossfader, so you can hear a deck the audience cannot."><PhonesIcon/></Toggle></div><Separator/></div>
        <div className="play-deck-transport" role="group" aria-label={`Deck ${index + 1} transport`}>
          <Toggle className="play-transport-button play-transport-play" on={d.playing ?? false} disabled={d.status !== 'ready' || !commands.setDeckPlaying} label={`Deck ${index + 1} play/pause`} title={commands.setDeckPlaying ? 'Play / Pause' : 'Deck playback is not connected yet'} onChange={playing => commands.setDeckPlaying?.(d.id, d.cueHeld ? true : playing)} hint="Plays or pauses this deck on its own, from where it left off. The header's play button moves every loaded deck at once.">{d.playing ? 'Ⅱ' : '▶'}</Toggle>
          <Momentary held={d.cueHeld ?? false} disabled={d.status !== 'ready' || !commands.cueDeck} label={`Deck ${index + 1} transport cue`} onHold={held => commands.cueDeck?.(d.id, held)} onTakeover={()=>commands.setDeckPlaying?.(d.id,true)}>CUE</Momentary>
          <Toggle className="play-transport-button" on={d.synced ?? false} disabled={d.status !== 'ready' || !commands.setDeckSync} label={`Deck ${index + 1} sync`} title={commands.setDeckSync ? 'Sync deck to the shared tempo and beat' : 'Deck playback is not connected yet'} onChange={synced => commands.setDeckSync?.(d.id, synced)} hint="Holds this deck to the shared tempo and to the beat the other decks are on. It needs a saved beat grid to know where its beats are; without one the deck plays at the speed it was recorded.">Sync</Toggle>
          {commands.beatJump && <div className="play-beat-jump" role="group" aria-label={`Deck ${index+1} beat jump`}><div className="wdg wdg-control-group play-beat-jump-pair">
            <ButtonFace size="medium" disabled={!d.gridAvailable} aria-label={`Deck ${index+1} jump forward one beat`} title="Advance active stems together by 1 beat" onClick={()=>commands.beatJump?.(d.id,1)}><span className="play-jump-stacked">↑</span><span className="play-jump-abreast">→</span></ButtonFace>
            <ButtonFace size="medium" disabled={!d.gridAvailable} aria-label={`Deck ${index+1} jump backward one beat`} title="Rewind active stems together by 1 beat" onClick={()=>commands.beatJump?.(d.id,-1)}><span className="play-jump-stacked">↓</span><span className="play-jump-abreast">←</span></ButtonFace>
          </div></div>}
        </div></div>
      </div>;
}
